...
function ListedTrails({ onLogout }) {
  ...
  if (!pharma) {
    return <div className='error-message'>U dont Have Access</div>;
  }
  
  const userPharmaId = pharma.pharmaId || pharma.id;
  ...
  const handleTrailCreated = (newTrail) => {
    axios.post('http://localhost:5000/trailDetails', newTrail)
      .then(response => {
        loadTrails();
      })
      .catch(error => {
    ...
    <div className='listed-trails-page'>
      <nav className='navbar'>
        <div className='navbar__profile'>
          <img src={pharma.profilePicture || 'https://via.placeholder.com/150'} alt={pharma.name} className='navbar__profile-pic' />
