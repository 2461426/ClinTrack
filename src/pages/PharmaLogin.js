...
import pharmaDetailsData from '../data/pharmaDetails.json';

function PharmaLogin({ onLogin }) {
    const [showPassword, setShowPassword] = useState(false);
    const [authError, setAuthError] = useState('');
    const navigate = useNavigate();
    const pharmaAccounts = pharmaDetailsData.pharmaDetails || [];
        ...
        <div className='flex items-center justify-center min-h-screen'>
            <div className='flex flex-col items-start gap-6'>
                <div className='flex flex-col items-start'>
                    <h1 className='text-3xl font-light'>Login to</h1>
                    <h1 className='text-4xl font-bold text-indigo-500'>Start creating trails...</h1>
                </div>

                <div className='w-auto h-auto border rounded-[25px]'>
                    <div className='flex justify-between items-center gap-4'>
                        <Lottie animationData={pharmaLoginAnimation} className='h-[300px] w-auto' />
                        ...
                        <form onSubmit={formik.handleSubmit} className='relative z-10'>
                            <div className='flex flex-col items-start p-6 gap-4'>
                                ...
                                <div className='flex flex-col gap-1 items-start w-full'>
                                    <h1 className='text-md font-semibold'>Email ID</h1>
                                    <input
                                        type="text"
                                        placeholder='Enter Email'
                                        className='border w-64 h-11 pl-4 rounded-lg border-gray-300 focus:outline-none'
                ...
                <button onClick={() => navigate('/LoginAs')} className='border font-regular text-gray-500 px-4 py-2 rounded-full hover:bg-indigo-500 hover:text-white' >Go Back</button>
            </div>
        </div>
    )
}
