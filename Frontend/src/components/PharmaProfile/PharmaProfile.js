...
function PharmaProfile() {
    ...
    useEffect(() => {
        const fetchPharmaProfile = async () => {
            try {
                ...
                // Fetch pharma details from data.json
                const response = await fetch('http://localhost:5000/pharmaDetails');
                const pharmaDetails = await response.json();
 
                // Find the logged-in pharma user
                const currentPharma = pharmaDetails.find(
                    pharma => pharma.email === user.email || pharma.pharmaId === user.id
                );
 
                if (currentPharma) {
                    setPharmaData(currentPharma);
 
                    // Fetch trails associated with this pharma
                    const trailsResponse = await fetch('http://localhost:5000/trailDetails');
                ...
                console.error('Error fetching pharma profile:', err);
                            ...
                            <div className="pharma-profile__trails-grid">
                                {trails.map((trail) => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <span className="pharma-profile__trail-category">{trail.category}</span>
